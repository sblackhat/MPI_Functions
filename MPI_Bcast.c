   ...
   while(cSerie < size){
      ...
      if(rank < participate){
        ...
        if(rank < cSerie){
          ...
          if (partner <= maxRank){
            MPI_Send(buffer, count, datatype, partner, 0, MPI_COMM_WORLD);
          }
        }else {
          MPI_Recv(buffer, count, datatype, partner, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
      }
   cSerie <<=1;
   participate <<=1;
